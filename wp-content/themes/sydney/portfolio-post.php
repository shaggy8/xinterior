	...
	<div class="content-area">
		<div id="container" class="site-content site-main">
			<div id="content" class="hentry">
				<div class="portfolio-nav_wrap">
					<?php  c2c_previous_or_loop_post_link() ?>
					<a href="/proekty"></a>
					<?php  c2c_next_or_loop_post_link(); ?>
				</div>
				<h2 class="text-center font-narrow margin-top-40">
					<?php global $post;
					$title = get_the_title();
						...
						<div class="entry">
							...
							<div class="portfolio_images_block">
								...
								while ( list( $key, $value ) = each( $array_post_thumbnail_id ) ) {
									...
									$image_desc		=	$image_desc->post_content;
									$single_image			=	wp_get_attachment_image_src( $post_thumbnail_id, 'portfolio-thumb' );
									$single_image_large	=	wp_get_attachment_image_src( $post_thumbnail_id, 'large' );

									if ( 0 == $key ) { ?>

										<div class="varx-carousel_wrapper">
											<!-- <span id='prev_slide' class="left-arrow"></span>
											<span id='next_slide' class="right-arrow"></span>
											<div class="carousel-line" data-current=0> -->
									<?php } ?>
					...
					<script type="text/javascript">
						(function($){
							$(document).ready(function(){
								...
								//         carousel = $('.carousel-line'),
								//         currentSlide = parseInt(carousel.data('current'));

								// 	carousel.width(carousel.children().size()*slideWidth);
								// 	carousel.find('.carousel-item').eq(1).addClass('active');
								...
								//         carousel.find('.carousel-item').eq(currentSlide + 1).addClass('active');
								//         if ( currentSlide <= carousel.children().size()) {
								//             carousel.css('margin-left',-(currentSlide-2)*slideWidth);
								//             carousel.append(carousel.children().first().clone());
								//             carousel.children().first().remove();
								//             currentSlide--;
								//         }
								//         carousel.animate({marginLeft: currentSlide*slideWidth}, 0).data('current',currentSlide);
								//     }
